...
def _sync_drission_login(email, password):
    """DrissionPage ကိုအသုံးပြု၍ Synchronous ဖြင့် Login ဝင်ခြင်း"""
    try:
        # Browser Options သတ်မှတ်ခြင်း
        co = ChromiumOptions()
        ...
        # Headless mode ကို False အနေနဲ့ အရင်စမ်းပါ။ အဆင်ပြေမှ True ပြောင်းပါ။
        co.headless(False) 
        ...
        # Google Login Popup (Tab အသစ်) သို့ ပြောင်းခြင်း
        page.wait.new_tab()
        ...
        # Order Page ရောက်သည်အထိ စောင့်ခြင်း
        page.wait.url_change("customer/order", timeout=30)
        
        # Cookies များကို ဆွဲထုတ်ခြင်း
...
async def auto_login_and_get_cookie():
    ...
    async with auth_lock:
        ...
        # DrissionPage သည် synchronous ဖြစ်၍ Bot ကို မပိတ်ဆို့စေရန် Thread ဖြင့် ခေါ်ယူခြင်း
                ...
                for pkg_data in packages_to_buy:
                    ...
                    for item in items:
                        ...
                        if res.get('status') == 'success':
                            ...
                            break # Error တက်လျှင် ထို Package ၏ နောက်ထပ် Item များကို ဆက်မဝယ်တော့ဘဲ ရပ်မည်
                            
                    # 🟢 တစ်စိတ်တစ်ပိုင်း အောင်မြင်မှုများကို သီးခြားခွဲထုတ်ခြင်း
                    ...
                    # 🟢 တစ်စိတ်တစ်ပိုင်း ကျရှုံးမှုများကို သီးခြားခွဲထုတ်ခြင်း
                    if pkg_fail_count > 0:
...
async def on_startup(bot: Bot):
    # Webhook URL ကို Telegram Server ဆီသို့ ချိတ်ဆက်ခြင်း
    await bot.set_webhook(WEBHOOK_URL)
    print(f"✅ Webhook successfully set to {WEBHOOK_URL}")
    
    # Background Tasks များကို စတင်ခြင်း
    print("Starting Heartbeat & Background tasks...")
...
async def on_shutdown(bot: Bot):
    # Bot ရပ်သွားတဲ့အခါ Webhook ကို ဖြုတ်ချခြင်း
    await bot.delete_webhook()
    print("🔴 Webhook removed and Bot shut down.")

def main():
    print("နှလုံးသားမပါရင် ဘယ်အရာမှတရားမဝင်")
    
    # Middleware များ ထည့်သွင်းခြင်း
    dp.message.middleware(ScamAlertMiddleware())
    
    # Startup နှင့် Shutdown လုပ်ငန်းစဉ်များကို မှတ်ပုံတင်ခြင်း
    dp.startup.register(on_startup)
    dp.shutdown.register(on_shutdown)
    
    # Aiohttp Web Server ဖန်တီးခြင်း
    app = web.Application()
    
    # Webhook Request များကို လက်ခံမည့် Handler ကို ဖန်တီးခြင်း
    webhook_requests_handler = SimpleRequestHandler(
        dispatcher=dp,
        bot=bot,
    )
    
    # Handler ကို Web Server ၏ Path သို့ ချိတ်ဆက်ခြင်း
    webhook_requests_handler.register(app, path=WEBHOOK_PATH)
    setup_application(app, dp, bot=bot)
    
    print("Bot is successfully running on Aiogram 3 Framework with Webhook... 🎉")
    # Web Server ကို စတင် Run ခြင်း
